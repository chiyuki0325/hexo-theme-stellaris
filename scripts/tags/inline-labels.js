/**
 * 修饰文本标签 v2 | https://github.com/chiyuki0325/hexo-theme-stellaris
 *
 * example:
 * {% psw 这是密码 %}
 */
...
hexo.extend.tag.register('sup', function (args) {
  ...
  var el = ''
  el +=
    '<sup class="tag-plugin colorful sup"' +
    ' ' +
    hexo.args.joinTags(args, ['color']).join(' ') +
    '>'
  el += args.text
  el += '</sup>'
  return el
...
hexo.extend.tag.register('sub', function (args) {
  ...
  var el = ''
  el +=
    '<sub class="tag-plugin colorful sub"' +
    ' ' +
    hexo.args.joinTags(args, ['color']).join(' ') +
    '>'
  el += args.text
