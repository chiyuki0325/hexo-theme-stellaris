        ...
        {(() => {
          if (site.categories && site.categories.length > 0) {
            if (page.category) {
              ...
              return (
                <a
                  className={page.layout === 'categories' ? 'active' : ''}
                  href={url_for(config.category_dir)}
                  key='categories'
                >
                  {__('btn.categories')}
        ...
        {(() => {
          if (site.tags && site.tags.length > 0) {
            if (page.tag) {
              ...
              return (
                <a
                  className={page.layout === 'tags' ? 'active' : ''}
                  href={url_for(config.tag_dir)}
                  key='tags'
                >
                  {__('btn.tags')}
          ...
          if (theme['post-index']) {
            ...
            for (let key of Object.keys(postIndex)) {
              elements.push(
                <a
                  className={
                    full_url_for(page.path).startsWith(full_url_for(obj[key]))
                      ? 'active'
                      : ''
                  }
