...
const RelatedPostsHelper = (props) => {
  ...
  elements.push(
    <section className='body' key='body'>
      <div className={cls}>{returnElements}</div>
    </section>
  )
  return elements
}
const RelatedPosts = (props) => {
  ...
  if (
    theme.article.related_posts.enabled &&
    !(
      theme.article.related_posts.disable_at_server_render &&
      theme.server_render.status
    )
  ) {
    return (
      <div className='related-wrap reveal' id='related-posts'>
        <RelatedPostsHelper
          title={__('meta.related_posts')}
          json={popular_posts_json(
            {
