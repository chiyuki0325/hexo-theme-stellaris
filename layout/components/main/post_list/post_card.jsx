module.exports = function PostCard(props) {
    ...
    if (postProperties.image || theme.article.auto_cover) {
      ...
      if (postProperties.image !== undefined) {
        if (postProperties.image.includes('/')) {
          ...
          cover_url =
            'https://source.unsplash.com/1280x640/?' + postProperties.image
        }
      } else {
        // 自动以 tags 作为关键词搜索封面
        if (post.tags) {
    ...
    if (post.pin) {
      elements.push(
        <span className='pin' key='pin'>
          <img src={url_for('/images/pin.svg')} alt='pin' />
        </span>
      )
    }
    return elements
    ...
    return (
      <div className='cover'>
        {(() => {
          ...
          elements.push(
            <img
              src={postProperties.image}
              alt={postProperties.image}
              key={postProperties.image}
            />
          )
          if (
            postProperties.headline ||
            postProperties.topic ||
            postProperties.caption
          ) {
            let style = {}
              ...
              >
                ...
                {(() => {
                  if (postProperties.headline) {
                    return (
                      <div className='title'>{postProperties.headline}</div>
                    )
                  }
                })()}
                {(() => {
  ...
  if (
    postProperties.image &&
    postProperties.image.length > 0 &&
    postProperties.headline !== undefined
  ) {
