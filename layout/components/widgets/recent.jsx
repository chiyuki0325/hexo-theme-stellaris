const Recent = (props) => {
  ...
  if (page.menu_id === 'wiki') {
    ...
    recentPosts = recentPosts.sort((p1, p2) =>
      p1.updated > p2.updated ? -1 : 1
    )
  } else {
              ...
              <span className='title'>
                {(() => {
                  ...
                  if (post.layout === 'wiki') {
                    ...
                    if (name) {
                      itemTitle.push(<strong key='wiki_name'>{name}</strong>)
                      itemTitle.push(<span className='dot' key='dot'></span>)
                    }
                  }
                  itemTitle.push(post.title || post.seo_title || post.wiki)
                  return itemTitle
                })()}
              </span>
            </a>
