...
const stellaris = {
    ...
    load: {
        ...
        fancyBox: () => {
            if (stellar.plugins.fancybox) {
                ...
                stellar.loadScript(stellar.plugins.fancybox.js, {defer: true}).then(
                    stellaris.init.fancyBox
                )
            }
        },
        search: () => {
            if (stellar.search.service && (stellar.search.service == 'local_search')) {
                stellar.loadScript(stellar.search.js, {defer: true}).then(
                    stellaris.init.search
                )
            }
        },
        copyCode: () => {
        ...
        */
        ...
        search: () => {
            if (stellar.search.service && (stellar.search.service == 'local_search')) {
                stellaris.jQuery(() => {
                    ...
                    })).observe(document.querySelector("div#search-result"), {childList: true})
                })
            }
        },
        swiper: () => {
        ...
        scrollReveal: () => {
            if (stellar.plugins.scrollreveal && ('ScrollReveal' in window)) {
                const selector = "body .reveal", sr = window.ScrollReveal()
                sr.destroy()
                ...
                sr.reveal(selector, {
                    distance, duration, interval, scale, easing: "ease-out"
                })
            }
        },
        lazyLoad: () => {
    ...
    initPageComponents: () => {
        ['toc', 'sidebar', 'clickEvents','relativeDate', 'registerTabsTag', 'outdatedCheck'].forEach(
            component => { stellaris.init[component]() }
        )
    },
    initPlugins: () => {
        ['scrollReveal', 'lazyLoad', 'fancyBox','swiper', 'search', 'themePlugins'].forEach(
            plugin => { stellaris.init[plugin]() }
        )
    },
    initOnFirstLoad: () => {
